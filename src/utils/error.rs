...
pub fn get_msg(reason_str: &String) -> String {
    ...
    if let Some(code) = raw.next() {
        ...
        match code {
            ...
            "07" => raw
                .next()
                .map_or(default, |name| format!("constructor {} not found", name)),
            ...
            "0C" => raw
                .next()
                .map_or(default, |name| format!("field {} already set", name)),
            "0D" => format!("expected named fields"),
            "0E" => format!("expected no fields"),
            "0F" => raw
                .next()
                .map_or(default, |var| format!("undefined variable {}", var)),
            ...
            "1A" => raw
                .next()
                .map_or(default, |ty| format!("enum {} not found", ty)),
            "1B" => raw
                .next()
                .map_or(default, |ctor| format!("constructor {} not found", ctor)),
            ...
            "1D" => raw
                .next()
                .map_or(default, |field| format!("field {} already set", field)),
            ...
            "1F" => raw
                .next()
                .map_or(default, |var| format!("variable {} not found", var)),
            ...
            "21" => raw
                .next()
                .map_or(default, |fname| format!("invalid field name {}", fname)),
            ...
            "27" => raw
                .next()
                .map_or(default, |ty| format!("type {} cannot be matched", ty)),
            ...
            "39" => raw
                .next()
                .map_or(default, |name| format!("function {} already defined", name)),
            "3A" => raw
                .next()
                .map_or(default, |name| format!("enum {} already defined", name)),
            ...
            "3E" => raw.next().map_or(default, |field_name| {
                raw.next().map_or(default2, |ctor_name| {
                    format!("fields {} already defined in ctor {}", field_name, ctor_name)
                })
            }),
            "3F" => raw.next().map_or(default, |name| {
                format!("enum {} must have at least one ctor", name)
            }),
            "40" => format!("unit type is not allowed"),
            "41" => raw
                .next()
                .map_or(default, |ty| format!("unknown type {}", ty)),
